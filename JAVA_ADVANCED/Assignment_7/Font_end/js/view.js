    ...
    Employees.forEach((employee, index) => {
        s += `
            <tr>
                ...
                <td >${index + 1}</td>
                <td style="cursor: pointer" onclick="controller.toDetailGroup(${employee.id})">${employee.name}</td>
            </tr>`
    });
    tBody.innerHTML = s
}
...
view.showModalUpdateGroupname = function () {
    ...
    document.getElementById('UpdateNewNameGroup').value = model.groupDetail.name
}
