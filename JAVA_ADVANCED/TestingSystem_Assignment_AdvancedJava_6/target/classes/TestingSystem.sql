...
DELIMITER $$
CREATE PROCEDURE DeleteExam (IN cID TINYINT)
BEGIN
	DECLARE eID TINYINT;

	SELECT ExamID INTO eID
    FROM exam WHERE CreatorID = cID;
    
    DELETE FROM examquestion WHERE ExamID IN (eID);
    DELETE FROM exam WHERE CreatorID = cID;
END$$
DELIMITER ;

DROP PROCEDURE IF EXISTS DeleteAccount;
DELIMITER $$
CREATE PROCEDURE DeleteAccount (IN cID TINYINT)
BEGIN
	DECLARE qID TINYINT;

	SELECT QuestionID INTO qID
    FROM question WHERE CreatorID = cID;
	
    DELETE FROM answer WHERE QuestionID = qID;
    DELETE FROM examquestion WHERE QuestionID = qID;
	DELETE FROM question WHERE CreatorID = cID;
    DELETE FROM groupaccount WHERE AccountID = cID;
    DELETE FROM employee WHERE AccountID = cID;
    DELETE FROM manager WHERE AccountID = cID;
    DELETE FROM `account` WHERE AccountID = cID;
END$$
DELIMITER ;
